...
export function SpaceBackground() {
  ...
  useEffect(() => {
    ...
    const drawStatic = () => {
      ...
      // Draw planets - static on desktop
      if (!isMobile) {
        ...
        // Draw connecting energy beams between planets - static
        ctx.globalAlpha = 0.3;
        for (let i = 0; i < planetsRef.current.length; i++) {
          ...
          gradient.addColorStop(0, p1.color + 'ff');
          gradient.addColorStop(0.5, p1.color + '80');
          gradient.addColorStop(1, p2.color + 'ff');
